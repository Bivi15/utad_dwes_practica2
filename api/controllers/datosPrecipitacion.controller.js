...
const deletePrecipitacion = async (req, res, next) => {
    try{
        const data = await datosPrecipitacionModel.findOneAndUpdate({ _id: req.params.id, isDelete: false }, { isDelete: true }, { new: true });
        if (!data) {
            return errorResponse(res, "Medición no encontrada", BAD_REQUEST);
        }
...
const getStats = async (req, res, next) => {
    try{
        ...
        const stats = await datosPrecipitacionModel.aggregate([
            { $match:  match },
            {
                $group: {
                    _id: null,
                    min: { $min: "$precipitacionAcumulada" },
                    max: { $max: "$precipitacionAcumulada" },
                    avg: { $avg: "$precipitacionAcumulada" },
                    all: { $push: "$precipitacionAcumulada" }
                }
            }
        ]);
        if (!stats[0])
